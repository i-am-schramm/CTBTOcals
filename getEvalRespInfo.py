...
with open('AMP.IU.SFJD.10.BHZ') as f:
   stage=f.read().split(' ')
stage2=float(stage[1])*1e-9 # may as well convert to nm
print(stage2)

os.system('evalresp SFJD BHZ 2018 1 1 1 1 -stage 3 2 -u dis -f /APPS/metadata/RESPS/RESP.IU.SFJD.10.BHZ')
with open('AMP.IU.SFJD.10.BHZ') as f:
   stage=f.read().split(' ')
stage3=float(stage[1])*1e-9 # may as well convert to nm
print(stage3)

### this should be made into a def that takes the stage and the sncl and outputs the gain
